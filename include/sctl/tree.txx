...
namespace sctl {
  ...
  template <class Real, Integer DIM, class BaseTree> void PtTree<Real,DIM,BaseTree>::GetScatterIdx(Vector<Long>& idx, const std::string& pt_name) const {
    const auto group = groups.find(pt_name);
    SCTL_ASSERT(group != groups.end());
    // The scatter carries each particle's handed position along with it.
    const Long Nloc = group->second.LocalCount();
    ScratchBuf<Long> pos(Nloc);
    #pragma omp parallel for schedule(static)
    for (Long i = 0; i < Nloc; i++) pos[i] = i;
    const Long Nsorted = group->second.SortedCount();
    if (idx.Dim() != Nsorted) idx.ReInit(Nsorted);
    group->second.ScatterForward((ConstIterator<Long>)pos.begin(), idx.begin(), 1);
  }
